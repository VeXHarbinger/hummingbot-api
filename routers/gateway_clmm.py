...
async def add_liquidity_to_clmm_position(
    ...
    Example:
        ...
        base_token_amount: 10
        quote_token_amount: 4.5
        slippage_pct: 1
        wallet_address: (optional)
    ...
    try:
        ...
        # Extract transaction hash from Gateway response
        # Gateway returns different formats: signature (Solana), transactionHash/txHash/hash (EVM chains)
        transaction_hash = (
            result.get("signature") or 
            result.get("txHash") or 
            result.get("hash") or
            (result.get("receipt", {}).get("transactionHash") if isinstance(result.get("receipt"), dict) else None)
        )
        
        if not transaction_hash:
            logger.error(f"No transaction hash found in Gateway response: {result}")
            raise HTTPException(status_code=500, detail=f"No transaction hash returned from Gateway. Response keys: {list(result.keys())}")
